    ...
    try {
      ...
      if (await link.isVisible()) {
        ...
        if (result.type === "download") {
          ...
          try {
            console.log("Popup opened, waiting for response...");
            const response = await popup.waitForResponse((response2) => {
              const contentType2 = response2.headers()["content-type"];
              return response2.url() === popup.url() || contentType2 && contentType2.includes("application/pdf");
            }, { timeout: 3e4 });
            const contentType = response.headers()["content-type"];
            console.log(`Popup response content-type: ${contentType}`);
            if (contentType && (contentType.includes("application/pdf") || contentType.includes("application/octet-stream") || contentType.includes("application/zip"))) {
              console.log("Detected file content in popup, saving...");
              ...
              console.log("Popup content is not a direct file. Checking URL...");
              const popupUrl = popup.url();
              if (popupUrl.endsWith(".pdf") || popupUrl.includes("visualizar")) {
                const context = page.context();
                const response2 = await context.request.get(popupUrl);
                const buffer = await response2.body();
                fs.writeFileSync(filePath, buffer);
                await popup.close();
                return { success: true, filePath };
              }
              await popup.close();
              return { success: false, error: `Popup opened with content-type: ${contentType}` };
            }
          } catch (error) {
