    ...
    async getCourseFiles(courseId: string, courseName?: string): Promise<{ success: boolean; files?: any[]; news?: any[]; error?: string }> {
        try {
            ...
            if (!formDataParts.some(p => p.startsWith('javax.faces.ViewState='))) {
                ...
                if (viewState) formDataParts.push(`javax.faces.ViewState=${encodeSigaa(viewState as string)}`);
            }

            const body = formDataParts.join('&');
            console.log(`[HttpScraper] Entering course ${courseId} via POST...`);
            const coursePageResponse = await axios.post(postUrl, body, {
                headers: {
