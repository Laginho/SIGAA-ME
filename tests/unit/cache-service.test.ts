...
// DATA-001: todo método é por conta; os casos abaixo usam sempre a mesma.
...
describe('CacheService', () => {
    ...
    it('returns an empty baseline for a course never seen before', () => {
        ...
        expect(service.getCourseState(ACC, 'x')).toEqual({ files: [], news: [] });
    });

    it('reports an empty diff once the baseline has been committed for the same ids', () => {
        const service = new CacheService();
        service.updateCourseState(ACC, 'c1', ['1', '2'], ['9']);

        const diff = service.diffCourseState(ACC, 'c1', [{ id: '1' }, { id: '2' }], [{ id: '9' }]);
    ...
    it('reports only the ids not present in the baseline', () => {
        const service = new CacheService();
        service.updateCourseState(ACC, 'c1', ['1'], []);

        const diff = service.diffCourseState(ACC, 'c1', [{ id: '1' }, { id: '2' }], []);
    ...
    it('discards a v1 file: an unscoped baseline cannot be attributed to an account (DATA-001)', () => {
        // Era o caso do BUG-009 (ids gravados com a quote de fechamento do JSF,
        // `555'`). A normalização na leitura morreu com o v1: o arquivo inteiro
        // é descartado, e todo arquivo v2 já nasce depois da correção do parser.
        storage.files.set(
        ...
        const service = new CacheService();
        const diff = service.diffCourseState(ACC, 'c1', [{ id: '555' }, { id: '556' }], [{ id: '9' }]);

        expect(diff.newFiles).toEqual([{ id: '555' }, { id: '556' }]);
        expect(diff.newNews).toEqual([{ id: '9' }]);
    });
    ...
    describe('forgetLastFile', () => {
        it('pops the last file from the first course that has files and returns it', () => {
            ...
            const result = service.forgetLastFile(ACC);

            expect(result).toEqual({ courseId: 'c1', fileId: '2' });
            expect(service.getCourseState(ACC, 'c1').files).toEqual(['1']);
            expect(service.getCourseState(ACC, 'c1').news).toEqual(['9']);
        ...
        it('skips courses with no files and returns the first course that has files', () => {
            const service = new CacheService();
            service.updateCourseState(ACC, 'c1', [], []);
            service.updateCourseState(ACC, 'c2', ['7'], []);
